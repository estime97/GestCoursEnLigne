...
namespace BlazorHero.CleanArchitecture.Infrastructure
{
    public class DatabaseSeeder : IDatabaseSeeder
    {
        ...
        public void Initialize()
        {
            AddAdministrator();
            AddBasicUser();
            AddProfile(RoleConstants.AgentCadastre, new List<string>
            {
                Permissions.Requisitions.View,
                Permissions.Requisitions.Export,
                Permissions.Requisitions.Import,
                Permissions.Requisitions.Update
            });
            AddProfile(RoleConstants.DCCF, new List<string>
            {
                Permissions.Requisitions.Synchronize
            });
            _db.SaveChanges();
        }
        ...
        private void AddProfile(string profile, List<string> permissions)
        {
            Task.Run(async () =>
            {

                var role = new BlazorHeroRole(profile, _localizer[$"{profile} role with permissions"]);
                var roleInDb = await _roleManager.FindByNameAsync(profile);
                if (roleInDb == null)
                {
                    await _roleManager.CreateAsync(role);
                    roleInDb = await _roleManager.FindByNameAsync(profile);
                    _logger.LogInformation(_localizer[$"Seeded {profile} Role."]);
                }
